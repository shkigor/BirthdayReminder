    ...
    <table>
        ...
        <tr>
            ...
            <g:sortableColumn property="patronymic"
                              title="${message(code: 'person.patronymic.label', default: 'Patronymic')}"/>

            <g:sortableColumn property="birthday"
                              title="${message(code: 'person.birthday.label', default: 'Birthday')}"/>
            ...
            <g:sortableColumn property="organization"
                              title="${message(code: 'person.organization.label', default: 'Organization')}"/>
        ...
        <g:each in="${personInstanceList}" status="i" var="personInstance">
            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                ...
                <td><g:link action="show"
                            id="${personInstance.id}">${fieldValue(bean: personInstance, field: "lastName")}</g:link></td>

                <td>${fieldValue(bean: personInstance, field: "firstName")}</td>

                <td>${fieldValue(bean: personInstance, field: "patronymic")}</td>

                <td><g:formatDate date="${personInstance.birthday}"/></td>

                <td>${fieldValue(bean: personInstance, field: "phone")}</td>

                <td>${fieldValue(bean: personInstance, field: "organization")}</td>
